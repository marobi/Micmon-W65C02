...
if len(sys.argv) < 2:
    print("usage: disasm6502.py rom.bin [mapfile] [labelfile]")
    sys.exit(1)

romfile   = sys.argv[1]
mapfile   = sys.argv[2] if len(sys.argv) > 2 else None
labelfile = sys.argv[3] if len(sys.argv) > 3 else None
...
if labelfile:
    with open(labelfile) as f:
        for line in f:
            ...
            addr = int(p[1][2:], 16)   # strip bank byte
            name = p[2].lstrip(".")

            # keep first label encountered for an address
    ...
    if size == 2 and i + 1 < len(rom):
        ...
        if mode == "imm":
            ...
            if dest in labels:
                ...
                operand = auto_labels.setdefault(dest, f"L{dest:04X}")
        ...
        if mode == "abs":
            if mnem in ("JSR", "JMP"):
                if addr in labels:
                    ...
                    operand = auto_labels.setdefault(addr, f"L{addr:04X}")
            else:
            ...
            if dest in labels:
                ...
                target = auto_labels.setdefault(dest, f"L{dest:04X}")
